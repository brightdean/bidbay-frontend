...
import { appName, bidsText, salesText } from '../strings'

const AppBar = ({ user, handleLogout, handleProfileClick, handleSalesClick, handleBidsClick }) => {
    return (
        <section className='flex items-center w-full bg-white py-4 px-6 max-h-[70px] justify-between border-b-[4px]'>
            ...
            <section
                className='flex items-center justify-center space-x-2 rounded-full px-3 py-2 cursor-pointer'
                onClick={handleSalesClick}
                >
                <span className='text-[16px] font-semibold tracking-wide'>{salesText}</span>
            ...
            <section
                ...
                >
                <span className='text-[16px] font-semibold tracking-wide'>{bidsText}</span>
